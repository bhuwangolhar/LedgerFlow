...
const styles = {
  ...
  statusBadge: (status: string) => ({
    ...
    fontSize: "12px",
    fontWeight: 600,
    backgroundColor: status === "paid" ? "#dcfce7" : status === "overdue" ? "#fee2e2" : "#dbeafe",
    color: status === "paid" ? "#166534" : status === "overdue" ? "#991b1b" : "#1e40af",
  }),
};
...
export default function InvoicesPage() {
  ...
  const fetchCustomers = async () => {
    try {
      const res = await api.get("/finance/customers", { params: { organization_id: selectedOrgId } });
  ...
  const handleCreateInvoice = async () => {
    setError("");
    if (!customerId || !invoiceDate || !dueDate || !totalAmount || !selectedOrgId) {
      setError("All fields are required. Please select a customer from the dropdown.");
      return;
    }
  ...
  return (
    <>
      <header style={styles.headerSection}>
        ...
        <h1 style={styles.moduleTitle}>Invoices</h1>
      </header>
      ...
      <div style={styles.card}>
        ...
        {customers.length === 0 && selectedOrgId && (
          <p style={{ fontSize: "12px", color: "#f59e0b", margin: "0 0 10px" }}>⚠️ No customers found. Please add a customer first.</p>
        )}

        <div style={styles.inputRow}>
          <input style={{ ...styles.input, marginBottom: 0 }} type="date" placeholder="Invoice Date" value={invoiceDate} onChange={(e) => setInvoiceDate(e.target.value)} />
        ...
        <button style={styles.button} onClick={handleCreateInvoice} disabled={!customerId || !selectedOrgId}>
          Add Invoice
        </button>
      </div>
        ...
        <table style={styles.table}>
          <thead>
            <tr>
              ...
              <th style={styles.tableHeader}>Invoice Date</th>
          ...
          <tbody>
            {invoices.map((invoice) => (
              <tr key={invoice.id}>
                ...
                <td style={styles.tableCell}>{invoice.Customer?.name || "N/A"}</td>
                <td style={styles.tableCell}>{new Date(invoice.invoice_date).toLocaleDateString()}</td>
                <td style={styles.tableCell}>{new Date(invoice.due_date).toLocaleDateString()}</td>
                <td style={styles.tableCell}>{formatCurrency(invoice.total_amount)}</td>
                <td style={styles.tableCell}>{formatCurrency(invoice.amount_paid)}</td>
                <td style={styles.tableCell}><span style={styles.statusBadge(invoice.status)}>{invoice.status}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
      ) : (
        <div style={styles.emptyText}>No invoices yet. Create one to get started!</div>
      )}
    </>
  );
}
